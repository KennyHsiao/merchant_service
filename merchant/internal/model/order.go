...
*/
func QueryOrderByOrderNo(db *gorm.DB, orderNo string, merOrderNo string, merchantCode ...string) (*types.OrderX, error) {
	txOrder := &types.OrderX{}
	if orderNo != "" || len(orderNo) > 0 {
		if err := db.Table("tx_orders").Where("order_no = ?", orderNo).Take(txOrder).Error; err != nil {
			return nil, errorz.New(response.DATABASE_FAILURE, err.Error())
		}
	} else if merOrderNo != "" || len(merOrderNo) > 0 {
		if err := db.Table("tx_orders").
			Where("merchant_code = ? ", merchantCode).
			Where("merchant_order_no = ?", merOrderNo).
			Take(txOrder).Error; err != nil {
			return nil, errorz.New(response.DATABASE_FAILURE, err.Error())
		}
	} else {
