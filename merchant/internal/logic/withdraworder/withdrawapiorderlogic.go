...
func (l *WithdrawApiOrderLogic) WithdrawApiOrder(req *types.WithdrawApiOrderRequestX) (resp *types.WithdrawApiOrderResponse, err error) {
	logx.WithContext(l.ctx).Infof("Enter withdraw-order: %#v", req)
	db := l.svcCtx.MyDB
	...
	if isOK, _ := redisLock.Acquire(); isOK {
		...
		} else if merchantCurrency.IsDisplayPtBalance != "0" && req.PtBalanceId == "" {
			return nil, errorz.New(response.PT_BALANCE_ID_REQUIRED, "商户子钱包模式，ptBalanceId必填")
		}

		// 檢查白名單
		if err = db.Table("mc_merchants").Where("code = ?", req.MerchantId).
			Where("status = ?", "1").
		...
		if isWhite := merchantsService.IPChecker(req.MyIp, merchant.ApiIP); !isWhite {
			logx.WithContext(l.ctx).Error("白名單檢查錯誤: ", req.MyIp)
		...
		if currencyCode == constants.CURRENCY_THB {
			if req.AccountNo == "" || len(req.AccountNo) < 10 || len(req.AccountNo) > 16 || !isMatch2 {
				logx.WithContext(l.ctx).Error("銀行卡號檢查錯誤，需10-16碼內：", req.AccountNo)
			...
			if req.AccountNo == "" || len(req.AccountNo) < 13 || len(req.AccountNo) > 22 || !isMatch2 {
				logx.WithContext(l.ctx).Error("銀行卡號檢查錯誤，需13-22碼內：", req.AccountNo)
				return nil, errorz.New(response.INVALID_BANK_NO, "AccountNo: "+req.AccountNo)
			}
		}

		req.WithdrawApiOrderRequest.OrderAmount = req.OrderAmount.String()
		//req.WithdrawApiOrderRequest.PtBalanceId = req.PtBalanceId.String()
		// 驗簽檢查
		if isSameSign := utils.VerifySign(req.Sign, req.WithdrawApiOrderRequest, merchant.ScrectKey, l.ctx); !isSameSign {
			logx.WithContext(l.ctx).Error("驗簽檢查錯誤: ", req.Sign)
			return nil, errorz.New(response.INVALID_SIGN)
		}

		orderAmount, errParse := req.OrderAmount.Float64()
		if errParse != nil {
		...
		if isExist {
			logx.WithContext(l.ctx).Error("訂單號重複錯誤: ", req.OrderNo)
		...
		if req.PtBalanceId != "" {
			ptBalanceId, _ := strconv.ParseInt(req.PtBalanceId, 10, 64)
			if ptBalanceId > 0 {
				withdrawOrder.PtBalanceId = ptBalanceId
			}
		}
		...
		if err != nil {
			logx.WithContext(l.ctx).Error("下發api提單失敗: ", err.Error())
			return nil, err
		}
		...
		if err := transactionLogService.CreateTransactionLog(l.svcCtx.MyDB, &types.TransactionLogData{
			...
			OrderNo:         orderWithdrawCreateResp.OrderNo,
