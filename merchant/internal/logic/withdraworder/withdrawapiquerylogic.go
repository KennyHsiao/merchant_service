...
func (l *WithdrawApiQueryLogic) WithdrawApiQuery(req *types.WithdrawApiQueryRequestX) (resp *types.WithdrawApiQueryResponse, err error) {
	logx.WithContext(l.ctx).Info("Enter withdraw-query: %#v", req)
	...
	// 檢查驗簽
	if isSameSign := utils.VerifySign(req.Sign, req.WithdrawApiQueryRequest, merchant.ScrectKey, l.ctx); !isSameSign {
