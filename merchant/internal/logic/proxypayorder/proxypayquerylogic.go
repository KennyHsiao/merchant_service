...
func (l *ProxyPayQueryLogic) ProxyPayQuery(merReq *types.ProxyPayOrderQueryRequestX) (*types.ProxyPayOrderQueryResponse, error) {
	logx.WithContext(l.ctx).Infof("Enter proxy-query: %#v", merReq)
	var resp types.ProxyPayOrderQueryResponse
	// 1. 檢查白名單、商户号，單號是否存在
	merchantKey, txOrder, errWhite := l.CheckMerAndWhiteList(merReq)
	if errWhite != nil {
		logx.WithContext(l.ctx).Error("商戶號及白名單檢查錯誤: ", i18n.Sprintf(errWhite.Error()))
		resp.RespCode = errWhite.Error()
		resp.RespMsg = i18n.Sprintf(errWhite.Error())
		return &resp, errWhite
	}
	merReq.ProxyPayOrderQueryRequest.AccessType = merReq.AccessType.String()
	// 檢查簽名
	checkSign := utils.VerifySign(merReq.Sign, merReq.ProxyPayOrderQueryRequest, merchantKey, l.ctx)
	if !checkSign {
	...
	//	logx.WithContext(l.ctx).Error("取渠道網官網錯誤: " + err.Error())
	//	return nil, err
	//}
	...
	//if errCHN != nil {
	//	logx.WithContext(l.ctx).Errorf("代付提單: %s ，渠道返回錯誤: %s, %#v", merReq.OrderNo, errCHN.Error(), &ProxyQueryRespVO)
...
func (l *ProxyPayQueryLogic) CheckMerAndWhiteList(req *types.ProxyPayOrderQueryRequestX) (merchantKey string, orderNo *types.OrderX, err error) {
	...
	//2.检查订单号是否存在
	order := &types.OrderX{}
	if order, err = model.QueryOrderByOrderNo(l.svcCtx.MyDB, "", req.OrderNo, req.MerchantId); err != nil || order == nil {
